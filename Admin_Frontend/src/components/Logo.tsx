export default function Logo({ size = 34, showText = true, dark = false }: { size?: number; showText?: boolean; dark?: boolean }) {
  return (
    <div className="flex items-center gap-2">
      <svg width={size} height={size} viewBox="0 0 120 120" fill="none" xmlns="http://www.w3.org/2000/svg" className="shrink-0">
        <defs>
          <linearGradient id="awh-mark-admin" x1="4" y1="4" x2="116" y2="116" gradientUnits="userSpaceOnUse">
            <stop stopColor="#FF3B4E" />
            <stop offset="1" stopColor="#8F0E22" />
          </linearGradient>
        </defs>
        <circle cx="60" cy="60" r="54" stroke="url(#awh-mark-admin)" strokeWidth="4" />
        <ellipse cx="60" cy="60" rx="54" ry="20" stroke="url(#awh-mark-admin)" strokeWidth="1.3" opacity="0.5" />
        <ellipse cx="60" cy="60" rx="28" ry="54" stroke="url(#awh-mark-admin)" strokeWidth="1.3" opacity="0.5" />
        <g fill="url(#awh-mark-admin)">
      ...
      {showText && (
        <div className="leading-[1.05] relative">
          <div className={`font-display font-black uppercase tracking-tight text-[15px] whitespace-nowrap ${dark ? "text-white" : "text-red-primary"}`}>
            <span>AN</span>
