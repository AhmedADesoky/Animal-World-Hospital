...
export default function ProfileAvatar({ size = 72 }: { size?: number }) {
    ...
    <div ref={wrapRef} className="relative inline-block" style={{ width: size, height: size }}>
      ...
      >
        {profile.avatar ? (
          ...
          <div className="w-full h-full bg-gradient-to-br from-red-bright to-red-deep flex items-center justify-center text-white font-display font-semibold" style={{ fontSize: size * 0.36 }}>
            {profile.name.charAt(0)}
          </div>
        )}
        <div className={`absolute inset-0 bg-charcoal/50 flex items-center justify-center transition-opacity duration-200 ${hover || menuOpen ? "opacity-100" : "opacity-0"}`}>
          <Camera size={size * 0.26} strokeWidth={2} className="text-white" />
        </div>
      </button>
      ...
      {menuOpen && (
        <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 w-44 rounded-xl bg-white border border-black/[.08] shadow-modal py-1.5 z-20 animate-[fadein_.15s_ease-out]">
          <button onClick={() => fileRef.current?.click()} className="w-full flex items-center gap-2.5 px-3.5 py-2.5 text-[13px] font-semibold text-charcoal hover:bg-grey-soft transition-colors">
            <Camera size={14} strokeWidth={2} /> {profile.avatar ? "Change photo" : "Upload photo"}
          </button>
          {profile.avatar && (
            <button onClick={removeAvatar} className="w-full flex items-center gap-2.5 px-3.5 py-2.5 text-[13px] font-semibold text-red-primary hover:bg-red-tint transition-colors">
